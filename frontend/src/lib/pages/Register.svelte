...
<div class="w-full max-w-[360px] border border-border bg-bg-subtle p-6">
  <h1 class="text-lg font-semibold text-text mb-4">Create account</h1>

  <form onsubmit={submit} class="space-y-4">
    <div>
      <label class="block text-sm text-text-muted mb-1" for="register-email">Email</label>
      <input
        id="register-email"
        type="email"
        bind:value={email}
    ...
    <div>
      <label class="block text-sm text-text-muted mb-1" for="register-password">Password</label>
      <input
  ...
  <p class="mt-4 text-sm text-text-muted">
    ...
    <a use:link href="/login" class="text-accent hover:underline">Sign in</a>
  </p>
